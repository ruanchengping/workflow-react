...
class ContentMore extends React.Component {
  getMenus = (menuArray) => {
    ...
    return menuArray.map(item => {
      return (
          <Timeline.Item style={{marginLeft: '0'}} key={item.key} className="home_timeline_item">
            <span style={{color: '#999', marginLeft: '0px', width: '160px', display: 'inline-block'}}>{item.time}</span>
    ...
    return (
      <div style={{overflow: 'auto'}}>
        <div>
          ...
          <div style={{float: 'right', color: '#7d7d7d', marginTop: '10px'}}><a>...更多</a></div>
    		</span>
        </div>
        <br/>
        <div>
