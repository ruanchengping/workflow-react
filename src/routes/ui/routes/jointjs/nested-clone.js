...
export default class NestedClone extends React.Component {
  render() {
    return (
        <div> 
            <h1>在单元格上旋停，看看嵌套图上克隆和图搜索的工作方式。</h1>     
            <div id="paper"></div>      
        </div>
    )
  ...
  componentDidMount(){
    var $info = $('<pre>');
    $info.css({ position: 'fixed', top: 150, right: 100 });
    $(document.body).append($info);
    // resetInfo();

    function resetInfo() {
        $info.text('在单元格上旋停，看看嵌套图上克隆和图搜索的工作方式。');
    }
