...
class EditRemind extends React.Component{
	state={
		checked:true,
	}
  ...
  componentDidMount = () =>{
    ...
    this.props.form.setFieldsValue({//需求界面的设计不全，表格和编辑消息界面内容不一致
		remindName:expRecord.remindName,
		flowReceiver:["creator","auditor","copier","supervisor"],
		orgReceiver:"person1",
		remark:"abc",
		remindEvents:"pass",
		remindMode:["邮件","email","sms","wechat","message"],
    });
  }
...
EditRemind.contextTypes = {
  store: React.PropTypes.object
};
const mapStateToProps = (state, ownProps) => ({
  designModel: state.designModel,
});
export default connect(mapStateToProps)(Form.create()(EditRemind));	
